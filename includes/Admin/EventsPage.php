...
class EventsPage {
	...
	public static function init() {
		...
		add_filter( 'logtrails_admin_submenus', array( __CLASS__, 'add_submenu' ) );
	...
	public static function add_submenu( $submenus ) {
		...
		$submenus[] = array(
			'page_title' => esc_attr__( 'Events', 'logtrail' ),
			'menu_title' => esc_attr__( 'Events', 'logtrail' ),
	...
	public static function render_events_page() {
		echo '<div id="logtrail-events-settings"></div>';
	}
	...
	public static function enqueue_scripts( $hook ) {
		...
		// Load asset file.
		$assets = require WPLT_ABSPATH . 'build/events-settings/index.asset.php';

		// Enqueue page styles.
		wp_enqueue_style(
			'logtrail-events-settings',
			WPLT_PLUGIN_URL . 'build/events-settings/index' . ( is_rtl() ? '-rtl.css' : '.css' ),
			array(),
			$assets['version']
		);
		...
		wp_enqueue_script(
			'logtrail-events-settings',
			WPLT_PLUGIN_URL . 'build/events-settings/index.js',
			$assets['dependencies'],
			$assets['version'],
			true
		);
		...
		// script translations.
		wp_set_script_translations( 'logtrail-events-settings', 'logtrail' );
	}
}
